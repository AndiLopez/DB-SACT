...
const baseUrl = 'http://192.168.0.12:4000/1.10';
// const baseUrl = 'http://192.168.43.238:4000/1.10';
...
export default class ScreenInsert extends Component {
    ...
    async addGeneral() {
        try {
            ...
            const response = await axios.post(baseUrl, { siglas_c, marca_r, modelo_r, amp_cd, hilo, amp_fase, no_rect, cd_tot_inst, tot_pta_cd, carga_act_cd, por_ocupacion_r, ac_actual_r });
            const { data } = response;
            console.log(data);
            // this.getProducts();
            this.clearInput();
    ...
    render() {
        const { lista, siglas_c, marca_r, modelo_r, amp_cd, hilo, amp_fase, no_rect, cd_tot_inst, tot_pta_cd, carga_act_cd, por_ocupacion_r, ac_actual_r } = this.state;
        return (
