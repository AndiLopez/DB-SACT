...
const urlGetG = 'http://192.168.0.12:4000/G';
// const urlGetG = 'http://192.168.43.238:4000/G';
...
export default class ScreenEliminarG extends Component {
    ...
    async deleteGeneral(siglas_central) {
        try {
            const response = await axios.delete(urlGetG + `${siglas_central}`);
            const { data } = response
            // console.log(data)
            this.getGeneral()
            this.myAlert()
                    ...
                    <View style={[styles.content]}>
                        <View style={styles.content2}>
                            ...
                            <View style={styles.Title}>
                                <Text style={styles.textTitle}>  Eliminar Información General </Text>
                            </View>
                            ...
                            <View style={styles.containerCard}>
                                {this.state.lista.map(central => (
                                    <View style={styles.containerCardGris}>
                                        <View key={central.siglas_central}>
                                            <View style={styles.containerInfo}>
                                                <Text style={styles.textNegritas}> Siglas Central:   </Text>
                                                <Text style={{ fontSize: 16 }}>{central.siglas_central}</Text>

                                            </View>

                                            <View style={styles.containerInfo}>
                                                <Text style={styles.textNegritas}> Nombre Central:   </Text>
                                                <Text style={{ fontSize: 16 }}>{central.nom_central}</Text>
                                            </View>
                                            ...
                                            <View style={styles.buttonEliminar}>
                                                <Button
                                                    color="red"
                                                    title="Eliminar"
                                                    onPress={() => this.deleteGeneral(central.siglas_central)}
                                                />
                                            </View>
                                        </View>
                                    </View>
